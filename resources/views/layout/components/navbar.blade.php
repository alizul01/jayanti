<nav class="bg-gray-700 text-white border-gray-200 md:px-16 py-1">
  <div class="flex flex-wrap items-center gap-8 mx-auto p-4">
      ...
      <ul
        ...
        <li>
          <a href="{{ route('dashboard') }}"
             class="block py-2 pl-3 pr-4 {{ Request::route()->getName() === 'dashboard' ? 'text-blue-500' : 'text-white' }} rounded md:bg-transparent md:p-0">Beranda</a>
        </li>
        <li>
    ...
    <div class="ml-auto">
      @if(auth()->check())
        <a href="{{ route('logout') }}"
           class="inline-block px-6 py-2 text-md rounded-lg font-medium leading-6 text-center text-white transition shadow hover:shadow-lg hover:bg-blue-800">
          Logout
        </a>
      @else
        <a href="{{ route('login') }}"
